...
record::record(const record& pRecord): mKey(pRecord.mKey), mValue(pRecord.mValue) {}
...
int record::get_key() const {
  return mKey;
}

int record::get_value() const {
  return mValue;
}
...
void link::set_key(int pKey) {
  mKey = pKey;
}

int link::get_key() const {
  return mKey;
}

node* link::get_node() const {
  return mID.get();
}
...
void node::set_parent(inner_node* pParent) {
  mParent = pParent;
}

inner_node* node::get_parent() const {
  return mParent;
}

void node::set_prev(node* pPrev) {
  mPrev = pPrev;
}
...
int leaf_node::get_key(int pIndex) const {
  return mRecords[pIndex].get_key();
}
...
node* leaf_node::join_sibling_node() {
  ...
  if (prev && prev->get_parent() == parent && prev->get_size() + size <= MAX_THRESHOLD) {
    // merge with prev
    merge_subject = prev;
    if (get_next()) {
  ...
  } else if (next && next->get_parent() == parent && next->get_size() + size <= MAX_THRESHOLD) {
    // merge wtih next
    merge_subject = next;
    if (get_prev()) {
...
inner_node::inner_node(const std::shared_ptr<node> &pHeir): node(false), mHeir(pHeir) {}

int inner_node::get_size() const {
  return mLinks.size();
}
...
int inner_node::get_key(int pIndex) const {
  return mLinks[pIndex].get_key();
}
...
// bplustree class ////////////////////////////////////////////////////////////
bplustree::bplustree() {
  mRoot = std::make_shared<leaf_node>();
}
...
bool bplustree::insert(const record& pRecord) {
  ...
  while (true) {
    ...
    if (isLeaf) {
      ...
      if (!lOriginal->is_too_big()) break; // break if no split is needed
      ...
      lNew->mRecords.insert(lNew->mRecords.begin(),
                            halfIter, endIter);
      ...
      if (!iOriginal->is_too_big()) break; // break if no split is needed
      ...
      iNew->mLinks.insert(iNew->mLinks.begin(),
                          std::next(halfIter, 1), endIter);
...
bool bplustree::remove(int pKey) {
  ...
  if (ltemp->get_size() > 0 && pKey < ltemp->get_key(0)) {
    ...
    hasSmallerKey = true;
  } else if (ltemp->get_size() == 0 && ltemp->get_next() && static_cast<leaf_node*>(ltemp->get_next())->get_size() > 0) {
    ...
    if (ip) {
      for (auto it = ip->mLinks.begin(); it != ip->mLinks.end(); it++) {
        if (it->get_node() == newTemp) {
          // std::cout << pKey << " should be the same as " << it->get_key() << std::endl;
  ...
  while (true) {
    ...
    if (isLeaf) {
      ...
      if (!lWillBeDeleted->is_too_small()) break;
      int key = lWillBeDeleted->get_key(0);
      ...
      if (iParent) {
        ...
        // }
        // std::cout << std::endl;
        leaf_node* lNew = static_cast<leaf_node*>(lWillBeDeleted->join_sibling_node());
        if (lNew) {
          // joined
          ...
          if (iParent->mHeir.get() == lWillBeDeleted) {
            ...
            for (auto it = iParent->mLinks.begin(); it != iParent->mLinks.end(); it++) {
              if (it->get_node() == lNew) {
      ...
      if (!iWillBeDeleted->is_too_small()) break;
      inner_node* iParent = iWillBeDeleted->get_parent();

      if (iParent) {
        iWillBeDeleted = static_cast<inner_node*>(temp);
        inner_node* iNew = static_cast<inner_node*>(iWillBeDeleted->join_sibling_node());
